...
from config import VanillaExperimentConfig, StochasticExperimentConfig, EXPERIMENT_TYPES
...
def get_activation_name(activation_class, activation_kwargs):
    ...
    if activation_kwargs:
        activation_name += "_" + "_".join(
            f"{k}_{v}" for k, v in activation_kwargs.items()
        )
...
def run_experiment(
    ...
    if isinstance(
        activation,
        (ThresholdActivation, JumpReLU, RescaledJumpReLU, FixedRescaledJumpReLU),
    ):
    ...
    if hasattr(model, "network"):
        ...
        for i, layer in enumerate(model.network):
            ...
            if isinstance(
                ...
                history[f"final_threshold_hidden_layer_{hidden_layer_count-1}"] = (
                    layer.get_threshold()
                )
            # Also check for activation in sequential modules
                ...
                for j, sublayer in enumerate(layer):
                    if isinstance(
                        ...
                        history[
                            f"final_threshold_hidden_layer_{hidden_layer_count-1}_sub_{j}"
...
class ExperimentTracker:
    ...
    def add_result(self, result: Dict[str, Any]) -> None:
        ...
        for key in result.keys():
            if key not in self.columns and (
                key.startswith("activation_")
                or key.startswith("layer_")
                or key.startswith("final_threshold")
                or key.startswith("threshold_hidden_layer_")
                or key in ["temperature", "dropout_prob"]
            ):
    ...
    def print_summary(
        self, format_spec: Dict[str, Dict[str, Any]] = None, tablefmt: str = "grid"
    ) -> None:
        """
        ...
        for col in all_columns:
            if col in format_spec:
                ...
                headers.append(
                    f"{prefix}{column_name.replace('_', ' ').title()}{suffix}"
                )
            else:
                column_name = col
                # Format activation_* and layer_* parameter column names for better readability
...
class ExperimentDefinition:
    ...
    def __init__(
        self, name: str, config_class: Type, base_params: Dict[str, Any] = None
    ):
        """
...
class ExperimentRunner:
    ...
    def run_grid_search(
        ...
        for activation_class, activation_kwargs in activations:
            activation_name = (
                activation_class.__name__ if activation_class else "NoActivation"
            )

            for hidden_dims in architectures:
                # For stochastic experiments, also loop through temperatures
                if (
                    self.experiment_def.config_class == StochasticExperimentConfig
                    and temperatures
                ):
                    for temp in temperatures:
                        ...
                        for key, value in history.items():
                            if (
                                key == "final_threshold"
                                or key.startswith("final_threshold_hidden_layer_")
                                or key.startswith("threshold_hidden_layer_")
                            ):
                                result[key] = value
