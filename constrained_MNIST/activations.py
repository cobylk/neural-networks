...
class ThresholdActivation(nn.Module):
    ...
    def forward(self, x):
        ...
        constrained_threshold = torch.clamp(
            self.threshold, self.constraint_min, self.constraint_max
        )
...
class RescaledJumpReLU(nn.Module):
    ...
    def __init__(
        self, initial_threshold=0.1, constraint_min=0.0, constraint_max=5.0, eps=1e-10
    ):
        super().__init__()
...
class EpochAwareMixin:
    ...
    def get_decay_factor(self, initial_value=1.0, final_value=0.0):
        ...
        if self.decay_schedule == "linear":
            ...
            normalized_decay = (decay - self.decay_rate**self.decay_epochs) / (
                1 - self.decay_rate**self.decay_epochs
            )
            return initial_value * normalized_decay + final_value * (
                1 - normalized_decay
            )
...
class DecayingRescaledReLU(EpochAwareMixin, RescaledReLU):
    ...
    def forward(self, x):
        ...
        if self.forward_count % 100 == 0 and torch.rand(1).item() < 0.01:
            print(
                f"[DEBUG] DecayingRescaledReLU: epoch={self.current_epoch}/{self.decay_epochs}, "
...
class DecayingRescaledJumpReLU(EpochAwareMixin, RescaledJumpReLU):
    ...
    def forward(self, x):
        ...
        if self.forward_count % 100 == 0 and torch.rand(1).item() < 0.01:
            print(
                f"[DEBUG] DecayingRescaledJumpReLU: epoch={self.current_epoch}/{self.decay_epochs}, "
    ...
    def extra_repr(self):
        ...
        return (
            f"{threshold_info}, initial_scale={self.initial_scale}, final_scale={self.final_scale}, "
            f"current_scale={current_scale:.2f}, decay_schedule={self.decay_schedule}, "
            f"current_epoch={self.current_epoch}/{self.decay_epochs}"
        )
...
class DecayingFixedRescaledJumpReLU(EpochAwareMixin, FixedRescaledJumpReLU):
    ...
    def forward(self, x):
        ...
        if self.forward_count % 100 == 0 and torch.rand(1).item() < 0.01:
            print(
                f"[DEBUG] DecayingFixedRescaledJumpReLU: epoch={self.current_epoch}/{self.decay_epochs}, "
